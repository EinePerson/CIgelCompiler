...
public:
    PreParser(Info* info) : m_alloc(1024 * 1024 * 4),m_info(info),m_parser() {
    ...
    void parse(Info* info){
        ...
        for(auto pair:m_unParsed){
            if(!pair.second){
                parse(pair.first);
                m_unParsed.insert(std::pair(pair.first,true));
            }
        }
    ...
    SrcFile* getFile(std::string name){
        std::string str = m_info->src->name;
        str += name;
    ...
    bool tryParseInclude(){
        if(peak().has_value() && peak().value().type == TokenType::include){
            ...
            m_file->includes.push_back(getFile(id));
